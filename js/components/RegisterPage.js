...
export class RegisterPage extends Component {
  constructor() {
	...
	checkValidUserName = (text) => {
	  ...
	  if (account_name === '') {
	    return { value: account_name, error: 'tips.register.errorNameNull' };
	  }
	  ...
	  const account = this.props.searchEntity.searchAccounts.filter(a => a[0] === account_name);
	...
	checkValidPassword = (text) => {
	  const password = text ? text.trim() : null;

	  const regex  =  new RegExp('(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[~!@#$%^&*_+=`|(){}:;<>,.?/\\[\\]-]).{8,63}');
  ...
  onCheckedPrivacy = () => {
 	...
 	onSubmit = (e) => {
 	  const {
 	    username, password, confirmPass, checkedPrivacy, errorName, errorPass, errorConfirm,
 	  } = this.state;
...
marginTop: 0, alignItems: 'flex-end', justifyContent: 'flex-start', backgroundColor: 'transparent',
       ...
       <View style={{
 flex: 1, backgroundColor: 'transparent', marginTop: 0, width: SCREEN_WIDTH * 0.8,
}}
       >
