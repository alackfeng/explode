...
class TransItem extends Component {
  ...
  render() {
    const {
      item, index, account, globalObject, dynGlobalObject,
    } = this.props;
    ...
    const transType = op[1].from === account;
...
class TableHistoryWrap extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const transHistory = nextProps.history ? nextProps.history.toJS() : null;

    if (transHistory && this.state.dataSource) {
      const historys = this.state && this.state.db ? this.state.db.slice(0) : [];
