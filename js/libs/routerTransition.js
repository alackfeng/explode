...
const apiLatenciesCount = 0; // Object.keys(apiLatencies).length;
// Actions
...
const filterAndSortURLs = (count, latencies) => {
  const urls = nodeList // SettingsStore.getState().defaults.apiServer
    .filter((a) => {
      /*
...
const willTransitionTo = (nextState, replaceState, callback) => {
  ...
  const connectionCheckPromise = !apiLatenciesCount ? connectionManager.checkConnections() : null;
  Promise.all([connectionCheckPromise]).then(((res) => {
    ...
    connectionManager.connectWithFallback(connect).then(() => {
      ...
      console.log(
        '=====[routerTransition.js]::App.willTransitionTo - connectWithFallback -  ',
      ...
      if (error.name === 'InvalidStateError') {
        ...
        const url = connectionManager.urls[0] || connectionString;
  ...
  if (connect && !apiLatenciesCount && !connectionCheckPromise) {
    connectionManager.checkConnections().then((res) => {
      ...
      if (res && res[0]) {
        const [latencies] = res;
        const urls = filterAndSortURLs(0, latencies);
