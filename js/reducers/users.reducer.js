  ...
  switch (action.type) {
    ...
    case TRIGGER_USERS_UNLOCK:
    {
      if (action.extra && (action.extra.type === 'open' || action.extra.type === 'close')) {
        ...
        const isLock = action.extra.type === 'lock';
    ...
    {
      const reg_status = (action.type === USERS_REGISTER.SUCCESS);
      return {
        ...state,
        entityReg: {
    ...
    {
      const login_status = (action.type === USERS_LOGIN.SUCCESS);
      return {
        ...state,
        entityLogin: {
    ...
    case USERS_UNLOCK.EVENT:
    {
      ...
      const unlock_status = (action.type === USERS_UNLOCK.SUCCESS);

      // 锁定不需要打开对话框
      const isLock = !!((state.entityUnLock.raw && state.entityUnLock.raw.extra && state.entityUnLock.raw.extra.type === 'lock'));
