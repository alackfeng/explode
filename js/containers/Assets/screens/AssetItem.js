...
class Asset extends Component {
  ...
  componentWillMount() {
	...
	updateAsset = (props) => {
	  ...
	  this.setState({
	    balanceObject, asset, symbol: asset && asset.get('symbol'), asset_name: asset && asset.getIn(['options', 'description']),
	  });
	}
	...
	onPressNavTo() {
	  ...
	  if (nav) {
	    this.props.nav('Transfer', { item, index });
	  }
	}
  ...
  onPress={this.onPressNavTo}
	...
	render() {
	  ...
	  const {
	    balanceObject, asset, asset_name, symbol,
	  } = this.state;
  ...
  <AssetItemWrap>
    <ListItem
      ...
      title={assetName}
      titleStyle={{
fontSize: 18, color: '#284159', fontWeight: 'bold', textAlignVertical: 'center',
}}
      subtitle={subTitle}
      ...
      avatarStyle={{
 backgroundColor: 'blue', height: 60, width: 100, borderRadius: 5,
}}
