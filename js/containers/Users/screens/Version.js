...
export class VersionInfo extends Component {
  ...
  constructor(props) {
    ...
    let buildVersion = version.split('-')[1];
  ...
  fetchAppVersion = (quiet = false) => {
    ...
    fetch( UpdateVer.url || 'https://www.pgyer.com/apiv2/app/check', {
      ...
      if(json.data) {
        ...
        let strToNum = (ver) => (ver.split('.')[0] * 10000 + ver.split('.')[1] * 1);

        let version = strToNum(json.data.buildVersion);
        let localVersion = strToNum(_This.state.buildVersion);
        ...
        let url = json.data.buildShortcutUrl;
